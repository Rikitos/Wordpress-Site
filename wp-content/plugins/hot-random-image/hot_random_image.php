...
class Hotrandom_image extends WP_Widget {
	...
	/**
	 * Shortcode {randomimage}images/random,100%,auto,Random image,https://www.hotjoomlatemplates.com{/randomimage}
	 */
	function Hotrandom_image_Render($srchtml) {
		...
		if (preg_match_all("#{randomimage}(.*?){/randomimage}#s", $srchtml, $matches, PREG_PATTERN_ORDER) > 0) {
			...
			foreach ($matches[0] as $match) {
				...
				{randomimage}images/random,100%,auto,Random image,https://www.hotjoomlatemplates.com{/randomimage}
				...
				*/
				...
				$images6 = glob($hotrandomimage_params[0].'/*.JPG');
				$images7 = glob($hotrandomimage_params[0].'/*.PNG');
				$images8 = glob($hotrandomimage_params[0].'/*.GIF');
				$images9 = glob($hotrandomimage_params[0].'/*.JPEG');
				$images10 = glob($hotrandomimage_params[0].'/*.SVG');
	...
	function widget( $args, $instance ) {
		...
		if($input == ''){
			...
			$images6 = glob($instance["folder"].'/*.JPG');
			$images7 = glob($instance["folder"].'/*.PNG');
			$images8 = glob($instance["folder"].'/*.GIF');
			$images9 = glob($instance["folder"].'/*.JPEG');
			$images10 = glob($instance["folder"].'/*.SVG');
		...
		<?php } ?>
				<img id="random-image-<?php echo esc_attr( $this->number ); ?>" class="hot-random-image" src="<?php echo esc_url( get_site_url() . '/' . $image ); ?>" alt="<?php echo esc_attr( $instance["alt"] ); ?>" />
	    <?php
			if($link){ ?>
				</a>
			<?php }
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'folder' ) ); ?>"><?php _e('Path to images:','hot_random_image'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'width' ) ); ?>"><?php _e('Width:','hot_random_image'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'height' ) ); ?>"><?php _e('Height:','hot_random_image'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'alt' ) ); ?>"><?php _e('Alt text:','hot_random_image'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php _e('Image link:','hot_random_image'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'userinput' ) ); ?>"><?php _e('Select specific images (optional)','hot_random_image'); ?></label>
